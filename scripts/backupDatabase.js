...
const destination = path.join(destinationRoot, `clipthat-${stamp}.db`);

db.backup(destination).then(() => {
  const retention = Math.max(1, Number(process.env.BACKUP_RETENTION) || 14);
  const backups = fs.readdirSync(destinationRoot).filter(name => /^clip(?:-vault|that)-.*\.db$/.test(name)).sort().reverse();
